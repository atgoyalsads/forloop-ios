//
//  LearnerDisplayNameViewController.swift
//  Forloop
//
//  Created by Tecorb on 30/03/20.
...
class LearnerDisplayNameViewController: UIViewController {
    ...
    func afterLoginGetDetailStatus(user:User){
        if self.isEdit{
            let LearnerDispVC = AppStoryboard.Learner.viewController(LearnerEditProfileViewController.self)
            LearnerDispVC.isEdit = self.isEdit
            self.navigationController?.pushViewController(LearnerDispVC, animated: true)
        }else{
...
extension LearnerDisplayNameViewController : UIImagePickerControllerDelegate,UINavigationControllerDelegate{
...
extension LearnerDisplayNameViewController{
