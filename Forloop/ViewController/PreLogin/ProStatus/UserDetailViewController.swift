...
class UserDetailViewController: UIViewController {
    ...
    @IBAction func nextTapped(_ sender: Any) {
        ...
        if validate.success {
            ...
            if user.fname == firstName && user.lname == lastName && user.zipcode == zipCode && user.countryCode == countryCode && user.contact == phoneNumber && user.dob == DOB && user.gender == gender{
...
//        }
    ...
    func moveToLinks() {
        let awsLinkVc = AppStoryboard.Main.viewController(AWSLinkViewController.self)
        self.navigationController?.pushViewController(awsLinkVc, animated: true)
    }
}

extension UserDetailViewController: UITableViewDataSource, UITableViewDelegate,UserContactCellDelegate {
