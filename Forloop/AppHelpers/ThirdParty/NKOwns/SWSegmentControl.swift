...
open class SWSegmentedControl: UIControl {
    ...
    private var items: [String] = ["Overview", "Reviews","Amenities"]
    
    
    // Wait for a day UIFont will be inspectable
    @IBInspectable open var font: UIFont = fonts.Roboto.bold.font(fontSize(rawValue: CGFloat(18))!) {
        didSet {
            self.configureView()
        }
    }
    ...
    private func initIndicator() {
        ...
        //self.selectionIndicatorView.applyGradient(withColours: [gradientViewColor.start,gradientViewColor.end], gradientOrientation: .horizontal, locations: [0.3,0.7])
